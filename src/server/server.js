...
const Backend = {
  ...
  withdrawPremium: async function (passenger) {
    if (web3.utils.isAddress(passenger)) {
      consortium.methods
        ...
        .then((result) => {
          console.log(
            "\tInsurance Premium has been withdrawn and credited to the insuree balance"
          );
        })
        .catch((error) => {
          console.log("\tError while withdrawing premium: " + error);
        });
    }
  },
  ...
  sendOracleResponse: async function (index, flightKey) {
    ...
    const consensus = await settings.methods
